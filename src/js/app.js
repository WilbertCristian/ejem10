...
function iniciarApp() {
    ...
    tabs(); //Cambia la sesion cuando se presiones los tabs
    ...
    idCliente();
    nombreCliente(); //añde el nombre del cliente al objeto de cita
...
function mostrarSeccion(){
    //ocultar la seccion que tenga las clase de mostrar
...
function mostrarAlerta(mensaje, tipo, elemento, desaparece = true) {
    //verificar si hay mas de una alerta
...
function mostrarResumen() {
    ...
    if(Object.values(cita).includes('') || cita.servicios.length === 0){
        mostrarAlerta('Hace flata datos o Servicios, Fecha u Hora', 'error', '.contenido-resumen', false);
    ...
    servicios.forEach(servicio => {
        ...
        const precioServicio = document.createElement('P');
        precioServicio.innerHTML = `<span>Precio</span> ${precio} bs`;
    ...
    const fechaFormateada = fechaUTC.toLocaleDateString('es-MX', opciones); //en Ingles 'en-US'
...
// 
    ...
    try {
        ...
        Swal.fire({
            icon: "error",
            title: 'Error',
            text: '¡Hubo un error al guardar datos,  Intente mas tarde!'
            // footer: '<a href="#">Why do I have this issue?</a>'
          }).then(() => {
            window.location.reload();
          });
